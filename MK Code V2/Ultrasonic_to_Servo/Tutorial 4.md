...
* Open the Serial Monitor and confirm and disance is diplayed and it changes as you move your hand closer or further away from the sensor. 
...
*	Now we need to initalize a Ultrasonic Senor object so add the following to the libaray initialization section


	NewPing sonar(triggerPin, echoPin, maxDistance);

* The triggerPin and echoPin you ahve seen before but the maxDistance you have not. This libaray requires that you define the maximum distance you will be measure. The sensor has a max distance of 400cm but for this case lets define a max distance of 200cm. So add 


	int maxDistance = 200;   // in centimeters

to the variable intialzation section of the code. 
...
The "delay(50)" guarntees that there will be enough time for the ultrasonic sensor to send and recieve a signal before the next request comes. The sonar.ping() call returns?? its measurement in microseconds between when the pulse was sent out and when it was recieved. This is a little hard to understand so it is then converted to distance in centimeter. 
* We do need to defince the "uS" and "distance" varaibles so lets add them to the variable initalization section.  
...
* Then we need to update the map function to reflect that we are reading the digial pins the ultrasonic sensor is connected to and not the analog input. So remove the line
...
* Finally we need to update our max and min values to reflect what the new sesnor will read. 
...
* You are done with tutroial! If you want see some more advacnced topics to you can go to tutorial 5 but that is optional. 
